 ...
 * <p>
 * {@link Sarsa4Agt} is an alternative to {@link TDNTuple4Agt}.
 * The differences between {@link Sarsa4Agt} and {@link TDNTuple4Agt} are:
 * <ul>
 * <li> {@link Sarsa4Agt} updates the value of a state for a player based on the value/reward
 ...
 * 		Thus {@link Sarsa4Agt} is much simpler to generalize to 1-, 2-, 3-, ..., N-player games
 * 		than {@link TDNTuple4Agt}.
 * <li> Eligible states: {@link Sarsa4Agt} updates with ELIST_PP=true, i.e. it has a separate
 * 		{@code eList[p]} per player p. {@link Sarsa4Agt} uses only one common {@code eList[0]}.
 * 		Only relevant for LAMBDA &gt; 0. 
 * </ul>
 * The similarities of {@link Sarsa4Agt} and {@link TDNTuple4Agt} are:
 * <ul>
 ...
 */
public class Sarsa4Agt extends NTuple4Base {
	...
	// is set to true in getNextAction2(...), if the next action is a randomly selected one:
	...
	/**
	 * Default constructor for {@link Sarsa4Agt}, needed for loading a serialized version
	 */
	public Sarsa4Agt() throws IOException {
		super();
	...
	public Sarsa4Agt(String name, ParTD tdPar, ParNT ntPar, ParOther oPar,
					 int[][] nTuples, XNTupleFuncs xnf, ArrayList<ACTIONS> allAvailActions, int maxGameNum) {
		super(name);
...
//		ScoreTuple scBest = new ScoreTuple(1); //this.getScoreTuple(NewSO, prevTuple);
	 ...
	 */
	private ACTIONS_VT adaptAgentSarsa(int nextPlayer, ScoreTuple R, NextState4 ns) {
		ACTIONS_VT a_next;
